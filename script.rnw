...
\label{sec:Introduction}
  ...
  \label{ssec:Terrorism}
    ...
    \subsection{Haddop}
      Haddop may have been appropriate as a dataset of my size would have benefited from more aspects of distributed computing. I produced my random tree using the drandomForest package that makes R run in a multithreaded workload. This however is only for one aspect of my script, and my compiling time could have been reduced if I could have distributed the load.
...
\label{sec:Enviroment Setup}
  ...
  \label{ssec:CreateSeed}
    ...
    <<Setting Document Seed for repetable results, echo=TRUE, eval=TRUE>>=
      set.seed(4205426)
    @
...
\label{sec:Data Analysis}
  ...
  \label{ssec:CorpusDTMWordcloud}
    ...
    \subsubsection{Corpus Creation Function}
      ...
      <<Corpus Building Function Used throughout program, eval=TRUE, echo=TRUE>>=
        buildCorpus <- function(someText){
          ...
          myCorpus <- tm_map(myCorpus, removeWords, c(stopwords("english"), customStopwords))
    ...
    \subsubsection{UK - Create Summary Dataframe}
      ...
      <<Create a subset of UK summary informaction, echo=TRUE, eval=TRUE>>=
  ...
  \subsection{Terrorist Groups and Bar Plots}
  
    \subsubsection{Preparing our Global Data}

      <<Count group attack occurences and plot, echo=TRUE, eval=TRUE>>=
    ...
    \subsubsection{Plotting Global Terrorist Group Activity}

      <<Plot Group Attack Occurences,eval=TRUE,echo=TRUE>>=
      ...
      \begin{figure}
        ...
        @
        \begin{center}
          \caption{Bar Plot: Bar Plot of Global Terrorist Group Activities, using ggplot2}
        \end{center}
        \label{fig:groupActivities}
      \end{figure}
    ...
    \subsubsection{Preparing our UK Data}
    
      \small
      <<Count UK group attack occurences, echo=TRUE, eval=TRUE>>=
    ...
    \subsubsection{Plotting UK Terrorist Group Activity}
      ...
      \begin{figure}
        ...
        @
        \begin{center}
          \caption{Bar Plot: Bar Plot of UK Terrorist Group Activities, using ggplot2}
        \end{center}
        \label{fig:UKgroupActivities}
      \end{figure}
...
\label{sec:RFPredictions}
  ...
  \subsection{Our Prediction}
  \label{ssec:OurPrediction}
    
    What I'm wanting to predict with my Prediction Model is the group name of a terrorist group, given a number of features about the attack they performed.
    
  \subsection{Our Method}
  \label{ssec:OurMethod}
  
    We will use Random Forrest to perform Our predictions.
    
    
  \subsection{Creating and Preparing a subset}
  \label{ssec:CreatePrepareSubset}
    
    We need to create a subset of data with the values that we wish to make our prediction from. I have chosen a set of values such as Region, number of people killed, the attack type, if the attack was successful, the main weapon used in the attack, the day of the attack, the month of the attack, the year of the attack, the target that they were attacking.
  
    \subsubsection{Create Subset}
    To create this subset, we will make use of the subset command and then partion the data from our main dataframe into our new data frame
      <<AllGroup subset Data Creation,eval=TRUE,echo=TRUE>>=
    ...
    \subsubsection{Omitting Null Values}
      Once we have all the data we wish to use in our model, we need to remove any null values to make sure that our dataset is feature complete. We will not be able to run a random forest if we have any missing information.\par To do this, we will make use of the na omit function again.
      <<AllGroup get rid of null values,eval=TRUE,echo=TRUE>>=
        my_df.AllGroups=na.omit(my_df.AllGroups)
      @
      
    \subsubsection{Creating and Limiting our Predicting Factor}
      A factor is a variable that has only a certain amount of pre-defined options. A group name is a factor as there are only a certain amount of terrorist groups in our dataset, and the purpose of our prediction is not to predict new terrorist groups that don't exist!\par We will have to limit the amount of factors our dataset has, which is a shame. This will meant that our prediction model will only be able to calculate the top 30 terrorist groups. This is because there is a limitation in random forest that a factor cannot have more than 31 different values.\par To be able to create only 30 factors in our dataset, we will make use of the tidyverse package and the fctlump function. This will allow us to retain 30 factors in our set and change the rest of the group names to Other.
      <<AllGroup Make gname Factor and limit to 30 groups,eval=TRUE,echo=TRUE>>=
        my_df.AllGroups$gname <- factor(my_df.AllGroups$gname)
        my_df.AllGroups$gname <- fct_lump(my_df.AllGroups$gname,
                                          n = 30)
      @
      
    \subsubsection{Sanitizing Group Name}
      We will make use of a Regular Expression to remove symbols and strange characters from group names. This will aid the accuracy of the model.
      <<AllGroup RegEX symbols w spaces,eval=TRUE,echo=TRUE>>=
        my_df.AllGroups$gname <- str_replace_all(my_df.AllGroups$gname,
                                               "[^[:alnum:]]",
                                               " ")
      @
      
    \subsubsection{Partitioning Unknown and Known Groups}
      Once our data has past many forms of sterilization, we will now partion our set to Known and Unknown Group names. We will want to Train our prediction model on Known group attacks and once we are confident about its accuracy, we will then attempt to predict the Unknown terrorist attacks.
      \small
      <<AllGroup Known and Unknown Dataset Creation,eval=TRUE,echo=TRUE>>=
    ...
    \subsubsection{Creating Known and Unknown Factors}
      We will now look at our data and see what information can be made into factors. This is to make sure that our dataset knows what values are predefined, and aids accuracy of our model.
      \small
      <<AllGroup Known Factors,eval=TRUE,echo=TRUE>>=
      ...
      \small
      <<AllGroup Unknown Factors, echo=TRUE, eval=TRUE>>=
    ...
    \subsubsection{Creating Test and Training Sets}
      We will now partition our Known Groups data set into training and testing Data frames. This is done making use of the creatDataPartion Command and we can specify that the split will be 70 percent Training, 30 percent testing.
      <<Create Training and Testing Set of Group data, , echo=TRUE, eval=TRUE>>=
      ...
      \subsection{Factors that Manipulate Model Accuracy}
      \label{ssec:FactorsModelAccuracy}
      
        \subsubsection{Number of Trees vs Model Accuracy}
          We will now explore areas that we can manipulate about our model and change settings on how our Random forest is generated. The amount of trees we generate can change the accuracy of our model, depending on our dataset. We will specify that our amount of trees will start at 100, then we will loop 10 times, increment our tree value 100 each time. We will take notes of our model accuracy, comparing it with our testing data and saving this information in a data frame for us to plot later.
          <<Number of trees Effect Random Forest Accuracy, echo=TRUE, eval=TRUE>>=
            trees.ntrees <- 100
        ...
        \subsubsection{Plot Number of Trees vs Model Accuracy}
          We will now plot our Number of trees, once again, making use of ggplot 2.
          <<Plot Random Forest Amount of Trees, echo=TRUE, eval=TRUE>>=
          ...
          \begin{figure}
            ...
            @
            \begin{center}
              \caption{Bar Plot: Bar Plot of nTree Accuracy when nTree incremented in value, using ggplot2}
            \end{center}
            \label{fig:ntreeAccuracy}
          \end{figure}
          ...
          <<nTree predictions to table, eval=TRUE, echo=FALSE, results="asis">>=
            print(xtable(trees.df,
                    caption="nTree Manipulation to see effect on Accuracy"))
            @
          
          The result of our test shows that the amount of trees that we use to make our prediction model with makes no preciveable difference to the accuracy. The lowest amount of trees in the graph could be argued to show worse accuracy, but these values are represented at a fraction of a percent.\par The disadvantage to using a large amount of trees in a prediction model the amount of time it takes for the model to be created. 
          
          
          
          
        \subsubsection{Number of mTry vs Model Accuracy}
          We will now see if the number of variables available for splitting at each node, the mTry value,  will make a difference in accuracy when incremented through stages from the value of 1, to the value of 5. We will use the same code as our last experiment, but instead of manipulating the nTree value, we will manipulate and observe the mTry Value.
          <<Number of mtry Effect Random Forest Accuracy, echo=TRUE, eval=TRUE>>=
            mtry.mtrys <- 1
            # create dataframe to store accuracies and corresponding number of trees
      ...
      \subsubsection{Plot Number of mTry vs Model Accuracy}
        Once we have gathered results, we will now plot a graph and see what our results come out at.
        <<Plot Random Forest Amount of MTRYS, echo=TRUE, eval=TRUE>>=
          mtry.accuracy.plot <- ggplot(mtry.df,
                                       aes(x=Mtrys,
                                           y=Accuracy))
          mtry.accuracy.plot <- mtry.accuracy.plot +
            geom_line() + geom_point()
          mtry.accuracy.plot <- mtry.accuracy.plot +
            xlim(1,mtry.mtrys)
          mtry.accuracy.plot <- mtry.accuracy.plot +
          ...
          mtry.accuracy.plot <- mtry.accuracy.plot +
            scale_color_calc()
          mtry.accuracy.plot <- mtry.accuracy.plot +
            ggtitle("Number of Mtrys vs Model Accuracy")
          mtry.accuracy.plot <- mtry.accuracy.plot +
            xlab("Mtrys (Number of Mtrys)")
          mtry.accuracy.plot <- mtry.accuracy.plot +
            ylab("Accuracy (percentage %)")
        @
        ...
        \pagebreak
          ...
          \begin{figure}
            <<mTry plot,eval=TRUE,echo=FALSE>>=
              mtry.accuracy.plot
            @
            \begin{center}
              \caption{Bar Plot: Bar Plot of mTry Accuracy when mTry incremented in value, using ggplot2}
            \end{center}
            \label{fig:mtryAccuracy}
          \end{figure}
        ...
        <<mTry predictions to table, eval=TRUE, echo=FALSE, results="asis">>=
          print(xtable(mtry.df,
                  caption="nTree Manipulation to see effect on Accuracy"))
        @
        
        As we can see, the quantity of mtry has a curve that saturates at a certain point, meaning that the model does not become more accurate with the increase of the number of variables available for splitting at each tree node. 
        
    \subsection{Creating Our Prediction Model}
    \label{ssec:CreatePredictionModel}
    
      \subsubsection{Creating the Model}
        Now we have made observations on how to improve the accuracy of our model, we will put this into practice and make predictions where groupname has an Unknown value.\par We will create our Random Tree with 200 trees and the value for mtry will be 3.  
        \small
      ...
      \subsubsection{Our Final Model Accuracy}
...
\small
<<Predicted Terrroist Groups excl Other catagory,eval=TRUE,echo=TRUE>>=
...
<<Create Bar Plot of all Predicted Terrorist Groups , echo=TRUE, eval=TRUE>>=
  ...
  my_df.predictedUnknowns.barplot <- my_df.predictedUnknowns.barplot +
    geom_bar(stat="identity") 
  my_df.predictedUnknowns.barplot <- my_df.predictedUnknowns.barplot +
    coord_flip()
  my_df.predictedUnknowns.barplot <- my_df.predictedUnknowns.barplot +
  ...
  my_df.predictedUnknowns.barplot <- my_df.predictedUnknowns.barplot +
    scale_colour_tableau()
  my_df.predictedUnknowns.barplot <- my_df.predictedUnknowns.barplot +
    ggtitle("Predicted Terrorist Groups and corrisponding Attacks")
  my_df.predictedUnknowns.barplot <- my_df.predictedUnknowns.barplot +
    xlab("Terorist Group (Name)")
  my_df.predictedUnknowns.barplot <- my_df.predictedUnknowns.barplot +
    ylab("Quantity of Predicted Incidents (incidents)")
@
...
@
        ...
        \pagebreak
          ...
          \begin{figure}
            ...
            @
            \begin{center}
              \caption{Bar Plot: Bar Plot of predicted Terrorist Groups, using ggplot2}
            \end{center}
            \label{fig:predictedTerroristUnknown}
          \end{figure}
          ...
          <<my_df.predictedUnknowns.freq predictions to table, eval=TRUE, echo=FALSE, results="asis">>=
            print(xtable(my_df.predictedUnknowns.freq,
                    caption="Frequency Table of Predicted Terrorist Groups"))
          @
          



<<Output Final Model Results, echo=TRUE, eval=TRUE>>=
